class ServerFoldersController < ApplicationController
  ...
  def destroy
    # Move folder's servers back to top level at the folder's position
  ...
  def build_folder_item(folder)
    folder.reload
    servers = folder.server_memberships.includes(:server).ordered.map(&:server)
    { type: :folder, folder: folder, servers: servers, position: folder.position }
  end
end
